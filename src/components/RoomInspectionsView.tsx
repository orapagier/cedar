...
export const RoomInspectionsView: React.FC = () => {
  ...
  const saveIndividual = (o: { id: string; name: string }) => {
    if (!canEdit) return;
    const c = occupantState[o.id] ?? { bedsOk: true, lockersOk: true, personalThingsOk: true };
    ...
    const { filed } = saveIndividualInspection({
      ...
      roomNumber: selectedRoom,
      bedsOk: c.bedsOk,
      lockersOk: c.lockersOk,
      personalThingsOk: c.personalThingsOk,
      score,
      status,
    ...
    setSavedMessage(
      filed
        ? `${o.name} saved individually — ${score}/100 ${status.toUpperCase()}.`
                      ...
                      {canEdit && (
                        <div className="px-3 py-2 flex items-center justify-between gap-2 border-t border-slate-800/70 bg-slate-900/40">
                          <span className="text-[10px] font-semibold text-slate-500">
                            {[c.bedsOk, c.lockersOk, c.personalThingsOk].filter(Boolean).length}/3 items
                          </span>
                          {individualFiledToday(o.id) ? (
                            ...
                            <button
                              ...
                              className="min-h-touch shrink-0 px-2.5 rounded-lg flex items-center gap-1.5 text-[11px] font-bold bg-amber-500 text-slate-950 hover:bg-amber-400 transition-colors active:scale-95"
                            >
