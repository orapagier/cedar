...
export const RoomInspectionsView: React.FC = () => {
  const { inspections, individualInspections, rooms, users, addInspection, saveIndividualInspection, canEdit, isSuperAdmin, currentUser } = useDorm();
  ...
  /** This resident's own rating for today, if one has been saved on his own. */
  const individualFiledToday = (studentId: string) =>
    individualInspections.find(i => i.studentId === studentId && i.date === today);
  ...
  /** Rate one resident on his own, aside from the whole-room record above. */
  const saveIndividual = (o: { id: string; name: string }, excused = false) => {
    if (!canEdit) return;
    const c = occupantState[o.id] ?? { bedsOk: true, lockersOk: true, personalThingsOk: true };
    const passed = Number(c.bedsOk) + Number(c.lockersOk) + Number(c.personalThingsOk);
    const score = excused ? 0 : Math.round((passed / 3) * 100);
    const status: 'pass' | 'warning' | 'fail' | 'excused' = excused
      ? 'excused'
      : score >= 90
        ? 'pass'
        : score >= 70
          ? 'warning'
          : 'fail';
    const { filed } = saveIndividualInspection({
      date: today,
      studentId: o.id,
      studentName: o.name,
      roomNumber: selectedRoom,
      bedsOk: excused ? true : c.bedsOk,
      lockersOk: excused ? true : c.lockersOk,
      personalThingsOk: excused ? true : c.personalThingsOk,
      score,
      status,
    ...
    setSavedMessage(
      filed
        ? excused
          ? `${o.name} saved as excused.`
          : `${o.name} saved — ${score}/100 ${status.toUpperCase()}.`
        : `${o.name} already has an individual rating today — the one on file stands.${
            isSuperAdmin ? ' Correct it with the pencil on the individual log below.' : ' Ask the Dean to correct it.'
          }`
    );
    setTimeout(() => setSavedMessage(null), 5000);
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {/* Individual Ratings Log — one entry per resident saved on his own */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h3 className="font-bold text-white text-sm">Individual Inspection Ratings</h3>
          <span className="text-xs text-slate-400">{individualInspections.length} entries</span>
        </div>
        {individualInspections.length === 0 && (
          <p className="p-6 text-center text-xs text-slate-500">
            No individual ratings yet — save a resident on his own inside the inspection form.
          </p>
        )}
        <div className="divide-y divide-slate-800/70 max-h-[420px] overflow-y-auto">
          {individualInspections.map(rec => (
            <div key={rec.id} className="p-3 sm:p-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <p className="font-semibold text-white text-sm truncate">{rec.studentName}</p>
                  <span className="text-[11px] text-slate-500 shrink-0">Room {rec.roomNumber}</span>
                </div>
                <span className={`shrink-0 font-bold px-2 py-0.5 rounded text-[11px] ${
                  rec.status === 'excused' ? 'bg-sky-950 text-sky-300' :
                  rec.status === 'pass' ? 'bg-emerald-950 text-emerald-300' :
                  rec.status === 'warning' ? 'bg-amber-950 text-amber-300' :
                  'bg-rose-950 text-rose-300'
                }`}>
                  {rec.status === 'excused' ? 'Excused' : `${rec.score}%`}
                </span>
              </div>
              {rec.status !== 'excused' && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {[
                    { label: 'Beds', ok: rec.bedsOk },
                    { label: 'Lockers', ok: rec.lockersOk },
                    { label: 'Items', ok: rec.personalThingsOk },
                  ].map(c => (
                    <span key={c.label} className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                      c.ok ? 'bg-emerald-950/70 text-emerald-300' : 'bg-rose-950/70 text-rose-300'
                    }`}>
                ...
                {roomOccupants.map(o => {
                  ...
                  return (
                    <div key={o.id} className="mb-2 bg-slate-950/60 rounded-xl border border-slate-800 overflow-hidden">
                      ...
                      {/* Each resident can be saved on his own — a separate,
                          per-person record — while the whole room keeps its own
                          walk below. */}
                      {canEdit && (
                        <div className="px-3 py-2 flex items-center justify-between gap-2 border-t border-slate-800/70 bg-slate-900/40">
                          {individualFiledToday(o.id) ? (
                            <span className="flex items-center gap-1.5 text-[10px] font-semibold text-emerald-300">
                              <Lock className="w-3 h-3" />
                              {individualFiledToday(o.id)?.status === 'excused' ? 'Saved excused' : 'Saved'}
                            </span>
                          ) : (
                            <div className="flex items-center gap-1.5 w-full">
                              <button
                                type="button"
                                onClick={() => saveIndividual(o, true)}
                                className="min-h-touch flex-1 sm:flex-none sm:px-3 rounded-lg flex items-center justify-center gap-1.5 text-[11px] font-bold border border-sky-700/60 bg-sky-950/40 text-sky-300 hover:bg-sky-900/40 transition-colors active:scale-95"
                              >
                                Excused
                              </button>
                              <button
                                type="button"
                                onClick={() => saveIndividual(o)}
                                className="min-h-touch flex-1 sm:flex-none sm:px-3 rounded-lg flex items-center justify-center gap-1.5 text-[11px] font-bold bg-amber-500 text-slate-950 hover:bg-amber-400 transition-colors active:scale-95"
                              >
