...
export const StudyHoursLibraryView: React.FC = () => {
  const { studyLogs, users, rooms, saveStudyLog, canEdit, isSuperAdmin, currentUser, settings } = useDorm();
  ...
  const submitStudent = (student: (typeof occupants)[number]) => {
    if (!canEdit) return;
    const { filed } = saveStudyLog(buildLog(student));
    flash(
      filed
        ? `${student.name} logged at ${formatTime12h(checkTime)}.`
        : `${student.name} is already logged for tonight — the check on file stands.${
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 sm:p-5 rounded-2xl">
        <div>
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
            ...
            <span className="text-xs bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-2 py-0.5 rounded-full font-medium shrink-0 whitespace-nowrap">
              Academic Focus
            </span>
          </div>
          ...
          {listed.map(student => {
            ...
            return (
              <div key={student.id} className="p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
                ...
                {filed ? (
                  // This evening already holds his study check; the Dean's
                  // pencil is what changes it.
                  <div className="flex items-center gap-2 sm:justify-end">
                    <span className={`px-2 py-1 rounded-md text-[11px] font-bold ${QUIET_META[filed.quietness ?? 'quiet'].chip}`}>
                      {QUIET_META[filed.quietness ?? 'quiet'].label}
                    </span>
        ...
        {canEdit && listed.length > 0 && (
          <div className="p-3 sm:p-4 border-t border-slate-800 sticky bottom-0 bg-slate-900/95 backdrop-blur">
            <button
              ...
              className="w-full min-h-touch bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-colors active:scale-[0.99] disabled:active:scale-100"
            >
            ...
            <p className="text-[11px] text-slate-500 text-center mt-2">
              Save each resident as he turns up, or sweep whoever is left. Tonight holds one study check per
              resident — a name already taken keeps the check it was given.
            </p>
          </div>
        )}
      </div>
