...
/**
 * The twelve record sections, grouped so a drill-down can show one tab's worth
 * at a time instead of the whole stack at once.
 */
export type RecordGroup = 'attendance' | 'daily' | 'conduct' | 'away';

interface OccupantRecordsPanelProps {
  studentId: string;
  /** How many entries each section shows before it stops. */
  limit?: number;
  /** One tab's sections, or every section at once (the parent view). */
  group?: RecordGroup | 'all';
  /** The four-tile summary above the sections; off when a host shows its own. */
  showStats?: boolean;
}
...
export const OccupantRecordsPanel: React.FC<OccupantRecordsPanelProps> = ({
  studentId,
  limit = 5,
  group = 'all',
  showStats = true,
  ...
  const show = (g: RecordGroup) => group === 'all' || group === g;
